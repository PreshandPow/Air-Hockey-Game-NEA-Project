...
def handle_client(conn, addr):
    with LOCK:
        ...
        if len(LOBBY1['queue2']) == 1:
            conn.send('you are player1'.encode(FORMAT))

        elif len(LOBBY1['queue2']) == 2:
            conn.send('you are player2'.encode(FORMAT))

        if len(LOBBY1['queue3']) == 1:
            conn.send('you are player1'.encode(FORMAT))

        elif len(LOBBY1['queue3']) == 2:
            conn.send('you are player2'.encode(FORMAT))
        ...
        try:
            ...
            if msg_length:
                ...
                if msg == DISCONNECT_MESSAGE:
                    ...
                    print(f'Removed player from queue {PLAYERS}')
                    connected = False
                    with LOCK:
                        remove_player(conn)
                    ...
                    try:
                        ...
                        # Handle puck position updates
                        if 'puck_x' in position_data and 'puck_y' in position_data:
                            puck_x, puck_y = position_data['puck_x'], position_data['puck_y']
                            print(position_data)

                            # Update only if position changes
                            if PUCK_POSITION['x'] != puck_x or PUCK_POSITION['y'] != puck_y:
                                with LOCK:
                                    PUCK_POSITION['x'], PUCK_POSITION['y'] = puck_x, puck_y
                                    print(f'Puck position changed to: {PUCK_POSITION}')
                                broadcast(json.dumps({'puck_x': puck_x, 'puck_y': puck_y}), None)  # ,conn)
            ...
            print(f'sent message that {conn} has disconnected')
            remove_player(conn)

            try:
                print(f'Broadcasting opponent disconnect to remaining players.')
                broadcast('od', conn)  # Notify the remaining player
            except Exception as e:
                print(f'Error broadcasting opponent disconnect: {e}')
...
def remove_player(conn):
    with LOCK:
        if conn in PLAYERS:
            PLAYERS.remove(conn)
            print(f'Removed player from queue {PLAYERS}')
        for lobby in [LOBBY1, LOBBY2, LOBBY3]:
            for queue in lobby.values():
                if conn in queue:
                    queue.remove(conn)
                    print(f'Removed player from queue {queue}')
